...
curl https://raw.githubusercontent.com/gopaljayanthi/testArgoProj/main/bg-no-instrumentation-template/trigger-analysis.sh -o trigger-analysis.sh
...
containername=$( kubectl -n "$pns" get deploy $line -o jsonpath='{.spec.template.spec.containers[0].name}')
sed -i "s/CONTAINERNAME/$containername/g" template.yaml
done < deploys.txt
